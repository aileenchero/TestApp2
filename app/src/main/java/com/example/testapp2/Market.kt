...
class Market : AppCompatActivity() {
        ...
        try {
            ...
            withContext(Dispatchers.Main) {
                ...
                rvImages.apply {
                    ...
                    layoutManager = LinearLayoutManager(this@Market)
                }
            }
        } catch(e: Exception) {
            withContext(Dispatchers.Main) {
                Toast.makeText(this@Market, e.message, Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun deleteImage(filename: String) = CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            withContext(Dispatchers.Main) {
                Toast.makeText(this@Market, "Successfully deleted image.",
    ...
    private fun uploadImageToStorage(filename: String) = CoroutineScope(Dispatchers.IO).launch {
        try {
            curFile?.let {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@Market, "Successfully uploaded image",
                        Toast.LENGTH_LONG).show()
                }
            }
        } catch (e: Exception) {
